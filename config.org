...
See:
- https://emacs.stackexchange.com/a/79976
...
content is visible. We don't want this behaviour so we set org-startup-folded to true

#+begin_src emacs-lisp
(setq org-startup-folded t)
#+end_src

Alternatively you can set the startup behaviour on a per-file basis by adding one of the
following startup directives to the org file:

#+begin_quote different directives for startup behaviour on a perfile bases
#+STARTUP: overview
#+STARTUP: content
#+STARTUP: showall
#+STARTUP: show2levels
...
Pressing '<s TAB' in org mode automatically used to automatically inserts a 'source code
block'. Newer versions of org don't do this.
- you can use ~C-c C-,~ instead.
- we add the behaviour back by requiring the 'org-tempo module
...
- After installing, MSYS2 MinGW 64-bit should be in your start menu. Launch that, which brings
  up a terminal, and search for packages using ~pacman -Ss aspell~
- There are several options. I ended up using the following two packages to install aspell and
  ...
  #+end_quote>
- Now you can enter ~which aspell~ in the same terminal to find it's location. It'll probably be
...
** Highlight Characters (Tabs))

The highlight characters package is useful to specifically highlight annoying tabs in a
buffer. 
